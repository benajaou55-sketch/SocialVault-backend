...
# ── Cache (désactivé pour les stories) ────────────────────────────────────────
...
def cookies_to_tempfile(cookies_str: str, platform: str) -> str:
...
def persist_cookies(cookies_str: str, platform: str):
    """
    Sauvegarde les cookies Android dans le fichier .txt permanent sur Render.
    Après la première connexion de l'utilisateur, toutes les requêtes suivantes
    (même sans cookies dans le body) utiliseront ce fichier.
    """
    ...
    try:
        with open(cf, "w") as f:
            f.writelines(lines)
        print(f"[SocialVault] ✅ Cookies {platform} persistés ({len(lines)-1} entrées)")
    except Exception as e:
        print(f"[SocialVault] ❌ Erreur persist cookies {platform}: {e}")
...
async def set_cookies(request: Request):
    try:
        ...
        persist_cookies(cookies_str, platform)
...
async def resolve_twitter(url):
    ...
    apis = [f"https://api.fxtwitter.com/status/{tid}", f"https://api.vxtwitter.com/status/{tid}"]
...
async def resolve_instagram(url: str, extra_cookies: str = ""):
    url      = clean_instagram_url(url)
    is_story = "/stories/" in url.lower() or "highlight" in url.lower()

    # Ne pas cacher les stories (elles expirent rapidement)
    if not is_story:
        cached = cache_get(url)
        if cached: return cached
    ...
    tmp = None
    try:
        # ── Cookies : priorité aux cookies Android fraîchement envoyés ────────
        if extra_cookies:
            persist_cookies(extra_cookies, "instagram")   # ← sauvegarde permanente
            tmp         = cookies_to_tempfile(extra_cookies, "instagram")
            cookie_file = tmp
        else:
            # Utiliser le fichier sauvegardé lors de la dernière connexion
            cookie_file = get_cookie_file("instagram")

        if not cookie_file and is_story:
            return {"success": False,
                    "error": "Connexion Instagram requise pour télécharger les stories."}
        ...
        if info.get("_type") == "playlist":
            ...
            if len(valid) == 1:
                ...
                result = {"success": True, "direct_url": item["url"],
                          "title": info.get("title", "Instagram"), "thumbnail": item["thumbnail"],
                          "duration": 0, "platform": "instagram", "ext": item["ext"],
        ...
        if "login" in err or "cookie" in err or "checkpoint" in err:
            return {"success": False,
                    "error": "Session expirée. Reconnectez-vous via l'application."}
    finally:
        if tmp and os.path.exists(tmp):
            try: os.unlink(tmp)
            except Exception: pass

    # Fallback SnapInsta (posts publics)
    if not is_story:
    ...
    return {"success": False,
            "error": "Story expirée ou privée." if is_story
                     else "Impossible de télécharger ce contenu Instagram."}

# ── Facebook ──────────────────────────────────────────────────────────────────
async def resolve_facebook(url: str, extra_cookies: str = ""):
    lower = url.lower()
    ...
    try:
        if extra_cookies:
            persist_cookies(extra_cookies, "facebook")
        ...
        if not is_story: cache_set(url, result)
        return result
