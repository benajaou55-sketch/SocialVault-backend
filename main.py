...
# ═══════════════════════════════════════════════════════════════════════════════
#
# STRATÉGIE FINALE (après diagnostic complet) :
#
# PROBLÈME RACINE : tikwm.hdplay retourne parfois un fichier M4A (audio seul)
# déguisé en .mp4. La détection par Content-Type ou magic bytes depuis Render
# ne fonctionne pas car les CDN TikTok bloquent les requêtes Range depuis les
# serveurs et mentent sur le Content-Type.
#
# SOLUTION :
#   1. yt-dlp est utilisé EN PREMIER pour tous les liens (courts et longs)
#      car il analyse les métadonnées et sélectionne TOUJOURS un vrai format
#      vidéo grâce au format selector "bestvideo+bestaudio/best[vcodec!=none]"
#   2. tikwm est utilisé EN FALLBACK avec UNIQUEMENT le champ "play" (SD)
#      → "play" est toujours un vrai MP4 contrairement à "hdplay"
#      → "hdplay" est définitivement banni car il est la source du bug audio
#   3. tikmate comme troisième fallback
#
# POURQUOI "play" et pas "hdplay" :
#   - "hdplay" = flux séparé haute qualité, souvent M4A audio-only sur Render
#   - "play"   = flux unique SD, encodé avec vidéo+audio dans un seul MP4
#   La perte de qualité est mineure (720p→540p) mais le fichier est garanti vidéo.
...
async def expand_tiktok_short_url(url: str) -> str:
    """Résout vt/vm/m.tiktok.com vers l'URL canonique @user/video/ID."""
    headers = {
        "User-Agent": (
...
async def _tiktok_ytdlp(url: str) -> dict:
    """
    yt-dlp avec sélection forcée d'un format avec flux vidéo.
    C'est la méthode la plus fiable — elle ne retourne jamais de l'audio seul.
    """
    try:
        opts = {
            "quiet": True,
            "skip_download": True,
            # Forcer un format avec flux vidéo explicite.
            # "bestvideo[ext=mp4]+bestaudio[ext=m4a]" = merge (nécessite ffmpeg)
            # Sur Render sans ffmpeg → fallback sur "best[vcodec!=none][ext=mp4]"
        ...
        # Chercher un format avec vcodec explicitement non-none
        ...
        if not video_formats:
            # Fallback : tous formats avec vcodec non-none (pas forcément mp4)
            video_formats = [
                f for f in formats
        ...
        # Dernier recours : URL principale (info.url)
        du = info.get("url", "")
        if du:
            ext = info.get("ext", "mp4")
            return {
                "success": True, "direct_url": du,
                "title": info.get("title", "TikTok"),
                "thumbnail": info.get("thumbnail", ""),
                "duration": int(info.get("duration") or 0),
        ...
        print(f"[yt-dlp tiktok] erreur : {e}")
    return {"success": False, "error": "yt-dlp tiktok failed"}

async def _tiktok_tikwm_sd(url: str) -> dict:
    """
    tikwm — utilise UNIQUEMENT le champ 'play' (SD, garanti MP4 vidéo).
    'hdplay' est définitivement exclu car il retourne de l'audio M4A sur Render.
    """
    try:
        ...
        if data.get("code") == 0:
            ...
            # Cas diaporama (images) — pas de problème audio ici
            imgs = vd.get("images", [])
            if imgs:
                return {
                    "success": True, "direct_url": imgs[0],
                    "title": vd.get("title", "TikTok"),
                    "thumbnail": imgs[0], "duration": 0,
                    "platform": "tiktok", "ext": "jpg",
                    "is_image": True, "all_images": imgs
                }

            # Utiliser UNIQUEMENT "play" (SD) — jamais "hdplay"
            play  = vd.get("play", "")
            title = vd.get("title", "TikTok")
            cover = vd.get("cover", "")
            ...
            if play:
                return {
                    "success": True, "direct_url": play,
                    "title": title, "thumbnail": cover,
                    "duration": dur, "platform": "tiktok",
                    "ext": "mp4", "is_image": False, "all_images": []
                }
    except Exception as e:
        print(f"[tikwm_sd] erreur : {e}")
    return {"success": False, "error": "tikwm_sd failed"}
...
async def _tiktok_tikmate(url: str) -> dict:
    try:
        ...
        if videos:
            ...
            if best:
                return {
                    "success": True, "direct_url": best,
                    "title": data.get("title", "TikTok"),
                    "thumbnail": data.get("cover", ""),
                    "duration": 0, "platform": "tiktok",
                    "ext": "mp4", "is_image": False, "all_images": []
                }
    except Exception:
        pass
...
async def resolve_tiktok(url: str) -> dict:
    ...
    # Résoudre les liens courts avant tout
    target = url
    if _is_short_tiktok(url):
        resolved = await expand_tiktok_short_url(url)
        if resolved != url:
            target = resolved

    # yt-dlp EN PREMIER (garantit un vrai flux vidéo)
    result = await _tiktok_ytdlp(target)

    # tikwm SD en fallback (si yt-dlp échoue, ex: rate limit Render)
    if not result.get("success"):
        result = await _tiktok_tikwm_sd(target)

    # tikmate en dernier recours
    if not result.get("success"):
        result = await _tiktok_tikmate(target)

    # Si l'URL originale était courte et que target a échoué, retenter sur l'URL originale
    if not result.get("success") and target != url:
        result = await _tiktok_ytdlp(url)
        if not result.get("success"):
            result = await _tiktok_tikwm_sd(url)
        ...
        for v in m.get("videos", []):
            if v.get("url"):
                result = {"success": True, "direct_url": v["url"], "title": t.get("text","X")[:100],
                          ...
                          "duration": 0, "platform": "twitter", "ext": "mp4", "is_image": False, "all_images": []}
...
async def _instagram_snapinsta(url: str) -> dict:
    try:
        async with httpx.AsyncClient(timeout=12, follow_redirects=True) as c:
            ...
            if not m: return {"success": False, "error": "snapinsta token failed"}
            ...
            if dr.status_code == 200:
                ...
                if vids:
                    return {"success":True,"direct_url":vids[0],"title":"Video Instagram",
                            "thumbnail":imgs[0] if imgs else "","duration":0,"platform":"instagram",
                            "ext":"mp4","is_image":False,"all_images":[],"carousel_items":[]}
                if imgs:
    ...
    try:
        async with httpx.AsyncClient(timeout=12, follow_redirects=True) as c:
            r = await c.post("https://instavideosave.net/", data={"url":url},
                             headers={"User-Agent":"Mozilla/5.0","Referer":"https://instavideosave.net/",
...
async def _facebook_getfvid(url: str) -> dict:
    try:
        async with httpx.AsyncClient(timeout=12, follow_redirects=True) as c:
            ...
            if not token_m: return {"success":False,"error":"getfvid token failed"}
            ...
            if vids:
                ...
                return {"success":True,"direct_url":best,"title":"Video Facebook",
                        "thumbnail":"","duration":0,"platform":"facebook","ext":"mp4","is_image":False,"all_images":[]}
    except Exception: pass
...
async def _facebook_savefrom(url: str) -> dict:
    try:
        async with httpx.AsyncClient(timeout=12) as c:
            r = await c.get(f"https://worker.sf-tools.com/savefrom.php?sf_url={url}", headers={"User-Agent":"Mozilla/5.0"})
            data = r.json()
            urls = data.get("url",[])
            if isinstance(urls,list) and urls:
                ...
                if video_url and ".mp4" in video_url:
                    return {"success":True,"direct_url":video_url,"title":data.get("meta",{}).get("title","Facebook"),
                            "thumbnail":data.get("thumb",""),"duration":0,"platform":"facebook","ext":"mp4","is_image":False,"all_images":[]}
    except Exception: pass
...
async def _facebook_ytdlp(url: str, extra_cookies: str = "") -> dict:
    ...
    try:
        ...
        opts = {"quiet":True,"no_warnings":True,"skip_download":True,"format":"best[ext=mp4]/best","socket_timeout":8,"retries":1}
        if cf:
            opts["cookiefile"] = cf
        ...
        if du:
            return {"success":True,"direct_url":du,"title":info.get("title","Facebook"),
                    "thumbnail":info.get("thumbnail",""),"duration":int(info.get("duration") or 0),
                    "platform":"facebook","ext":ext,"is_image":ext in ["jpg","jpeg","png","webp"],"all_images":[]}
    except Exception: pass
    finally:
        if tmp and os.path.exists(tmp):
            try: os.unlink(tmp)
...
async def resolve_facebook(url, extra_cookies=""):
    ...
    if "login" in err or "cookie" in err: return {"success":False,"error":"Connexion Facebook requise."}
    if "private" in err: return {"success":False,"error":"Ce contenu Facebook est privé."}
...
async def resolve_video(req: ResolveRequest):
    ...
    try:
        if "tiktok.com" in url.lower():
            ...
            if cached: return cached
            async def _generic():
                opts = {"quiet":True,"skip_download":True,"format":"best[ext=mp4]/best","socket_timeout":8,"retries":1}
                try:
                    info = await run_yt_dlp(opts, url)
                    ext = info.get("ext","mp4")
                    du = info.get("url","")
